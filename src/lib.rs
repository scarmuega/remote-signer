        ...
        loop {
                ...
                .await
            {
                Ok(channel) => {
                    ...
                    let response: tonic::Response<tonic::codec::Streaming<ServerMessage>> =
                        client.connect_to_server(outbound).await?;
                    let inbound: tonic::codec::Streaming<ServerMessage> = response.into_inner();
